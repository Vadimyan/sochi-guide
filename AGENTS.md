...
- **Analytics:** Yandex Metrika (counter ID in `src/lib/metrika.ts`, shared by the client component `src/components/analytics/YandexMetrika.tsx` and the `<noscript>` pixel in `layout.tsx`). The component is disabled outside production builds and manually reports SPA route changes as hits (static export = client-side navigation). Outbound Yandex Maps links go through `src/components/analytics/TrackedLink.tsx`, which fires the `open-place-map` goal (label = place id) and `open-maps-collection` goal (label = region slug) plus matching visit params — use it for any new tracked link. Don't import values from `'use client'` modules into server components — they arrive as client references, not values (this exact bug produced an `[object Object]` URL once).
